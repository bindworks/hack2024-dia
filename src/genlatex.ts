...
const stddevGlucosePercent = jsonData.stddevGlucose && jsonData.averageGlucose && (jsonData.stddevGlucose / (jsonData.averageGlucose / 100));
...
\\begin{table}[h]
    ...
    \\begin{tabular}{|r|l|}
        ...
        \\hline
        Průměrná glukóza & ${jsonData.averageGlucose?.toFixed(2) ?? '-'} mmol/l \\\\
        Směrodatná odchylka & ${jsonData.stddevGlucose?.toFixed(2) ?? '-'} mmol/l (${stddevGlucosePercent?.toFixed(2) ?? '-'}\\%) \\\\
